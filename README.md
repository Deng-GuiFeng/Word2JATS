...
- **结果**:Layer-1 宏平均综合分 **0.967**——这是 10 样例混合宏 = 主样例 1–5 对**委员会金标准**宏 **0.936** + 补充样例 S1–S5 对**独立伪标签**宏 **0.998**(伪标签近满分会抬高均值,故对金标准的真实成绩是 0.936);样例 3 对金标准满分 1.000。少数维度低分(如样例 2 的 back=0、样例 1/5 的表)经 Layer-2 源核对**确认为出版层差异、非我方缺陷**(金标准发表时新增了源里没有的声明节、把整表拆分重编号)。
- **消融**(主 5 样例对金标准口径,full 宏 0.937):逐个关闭组件用 Layer-1 量化贡献——闭环整体 +0.019;章节结构修正在样例 1 上 0.905→0.946;声明拆分在样例 5 上 0.833→0.870;内容补全在这批规范样例上几乎不触发(它是缺内容时才用的安全网,如实标注)。
