...
访问 http://127.0.0.1:8000，选择 DeepSeek 或 Qwen，上传 Word。转换后直接查看预览、按需修改和下载。需要重新识别时，在结果页的“更多”中选择“重新转换”；新任务不会覆盖当前结果。
...
| 路径 | 内容 |
|---|---|
| 技术方案说明书.pdf / .docx | 技术方案的阅读与编辑版本 |
| 可运行原型/src/word2jats/ | 转换器、提示词、本地 DTD、公式样式表与期刊配置 |
| 可运行原型/webapp/ | 网页服务及预览资源 |
| 可运行原型/README.md | 独立安装、配置与使用说明 |
| 可运行原型/requirements.txt、.env.example 等 | 依赖、配置模板、项目安装与容器配置、许可 |
| 文件清单.json | 代码版本、依赖版本与文件 SHA-256 |

提交包不附实验稿件、结构参考、历史输出、模型响应缓存、真实密钥或内部开发文档；这些不是运行依赖。独立原型的简明说明见[原型运行说明](docs/原型运行说明.md)。
