...
def _split_persons(s, style):
    ...
    if style == "surname-first":
        ...
        for p in parts:
            ...
            if len(bits) == 2:
                out.append((bits[0], bits[1].rstrip(".")))
            else:
                return None, etal
        ...
        for p in re.split(r",\s*|\s+and\s+|\s*&\s*", s):
            ...
            if re.fullmatch(r"(?:[^\Wa-z\d_]\.?){1,4}", toks[-1]):
                ...
                continue
            k = len(toks) - 1
            while k > 0 and toks[k - 1].lower() in ("de", "van", "von", "del", "della",
                                                    "da", "dos", "den", "der", "di", "du"):
                k -= 1
...
_YVP = re.compile(r"(?P<year>(?:19|20)\d{2})\s*;\s*(?P<vol>\d+)"
                  r"(?:\s*\(\s*(?P<issue>[^)]+?)\s*\))?"
                  r"\s*:\s*(?P<fp>[A-Za-z]?\d+)(?:\s*[-–]\s*(?P<lp>\d+))?")
# APA: "(2023)."
...
def parse_vancouver(text, person_style):
    ...
    if cut is None:
        return None
    title, source = rest[:cut].rstrip(". "), rest[cut:].strip()
...
def parse_apa(text):
    ...
    # "Journal of Environmental Chemical Engineering, 11, 109746"
    bits = [b.strip() for b in src_part.rstrip(".").split(",")]
    source = bits[0] if bits else ""
    vol = fp = lp = None
    nums = [b for b in bits[1:] if b]
    if nums:
        if re.fullmatch(r"\d+", nums[0]):
            vol = nums[0]
        if len(nums) > 1:
            mm = re.fullmatch(r"([A-Za-z]?\d+)(?:\s*[-–]\s*(\d+))?", nums[1])
            if mm:
                fp, lp = mm.group(1), mm.group(2)
    d = {"persons": persons, "etal": etal, "article-title": _clean(title),
         "source": _clean(source), "year": m.group("year"),
         "volume": vol, "fpage": fp, "lpage": lp, "issue": None,
         "uri": None, "publication-type": "journal"}
    return d
...
def parse(key, text):
    ...
    try:
        return parse_apa(text) if kind == "apa" else parse_vancouver(text, pstyle)
    except Exception:
        return None
