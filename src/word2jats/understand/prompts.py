...
所有任务只允许模型返回源摘抄和语义关系；摘抄还要经过 ``ground.py`` 回到源文验证，故模型文字永远不直接进入输出。
"""
...
PREAMBLE = r"""You analyze the logical structure of an English scholarly manuscript. The input is a complete, addressable view of a Word document: [node] text [node.2] continuation after a soft line break [node|table] followed by [node.rN] rows whose cells are separated by ⇥ ⟦image#oN⟧, ⟦formula#oN⟧, and ⟦object#oN⟧ are source objects.
...
HEAD_BOUNDARY_SYSTEM = r"""为两个相互独立的文首处理任务确定输入范围。只判断范围，不提取字段，不改写原文，也不生成 XML。
...
FRONT_CONTENT_SYSTEM = r"""识别 Word 稿件中摘要和关键词的结构，并把结果指回 Word 原始记录。只判断内容的角色、边界、顺序和关系；不改写稿件文字，也不直接生成 XML。
...
姓名、学位、ORCID、邮箱和个人地址可以出现在不同记录中；只有 Word 原文明确写出或标出它们属于某位作者或编辑时，才归入该 `contrib`。ORCID 使用 `contrib-id-type="orcid"`，内容为标准 `https://orcid.org/` URI。只有 Word 原文直接声明某位作者或编辑具有通信身份时，才能使用 `contrib/@corresp="yes"`。
...
Word 中印有关系标记时，在相应 `xref` 中原样保留，并根据 `styles` 使用 `sup` 或 `sub`。标记在单位文字之前就保留在之前，在之后就保留在之后。Word 原文明确表达了关系但没有印出标记时，使用没有可见内容的 `xref`。

单位、个人地址和通信说明即使含有相同文字，也不得相互覆盖。一份完整的通信说明对应一个 `corresp`；不得拆分、合并、摘要、换序或改写通信说明。`corresp` 的文字只取自本身构成通信说明的 Word 记录；不得使用单位或其他个人地址记录扩写它。Word 中有完整文字的共享作者注释使用 `fn`，并只与原文标记指明的作者建立关系。只有标记而没有注释文字时，不得补写注释或生成空注释。
...
TASK: adjudicate two independent bibliography segmentations. You receive the source view and both candidates. Re-read the source; do not vote by majority. Produce the correct ordered set of unique verbatim entry-head excerpts. Any boundary that remains genuinely undecidable must be listed in issues and not silently chosen.
