...
每条物理行以 `ROW 0` 一类编号开头，方括号内是这条物理行的来处，形如 `[doc/p50:0-62]`：冒号之前是记录地址，之后是这条行在该记录中占据的字符区间。其后是它的完整可见文字。程序已按制表符把每条物理行机械切分，`⟦r0s0⟧` 一类标记是程序为切出的片段所编的号，不是稿件文字；`⇥` 代表原文中的一个制表符，片段内部不含制表符。
