...
def test_gold_inventory_is_frozen_and_complete():
    ...
    assert (element_count, len(attribute_values), media_count, mixed_slots) == (
        ...
        # 只是在同一个父元素内换了子元素次序。
        #
        # 元素数从 31015 降到 31014：删掉 05 的 <kwd-group><title>Key words</title>。
        # Word 里那行写的是「Key words: 」，与其余 13 例的「Keywords」同为引导词，
        # 那 13 例一律丢弃、只有 05 留成了标题；05 自己的上线版本也没有这个标题。
        # 属性取值、媒体数不变；title 元素在 sec 等处仍在使用，标签集合也不变。
        31014, 1510, 103, 2853,
    )
